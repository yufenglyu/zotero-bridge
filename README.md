...
- “索引链接”页：同步 Zotero 文献、维护本地 SQLite FTS5 索引、生成链接文件，并在“运行概览”中集中查看索引、链接、Zotero 实例和任务队列状态。
...
链接文件目录、命名模板和 URI 模板都在“索引链接”页的“链接文件”区域修改，输入后会自动保存。链接文件数量、健康状态和任务队列在“运行概览”中查看。修改目录、命名模板或 URI 模板后，点击“刷新链接”可按当前索引重新补写、覆盖、改名和清理链接文件。
...
“运行概览”保留三类核心信息：索引条目数和最近同步时间、链接文件数量和健康状态、当前 Zotero 实例和文献库数量。具体文献库版本显示在“文献库状态”中；待写入和失败任务显示在任务队列中。
